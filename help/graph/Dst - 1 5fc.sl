...
Voicer(16) { :e |
	...
	var samp = TrExpRand(tr, 0.07, 0.5);
	var root = TrChoose(tr, [23, 30, 35, 37]);
	var atk = TrExpRand(tr, 0.04, 0.3);
	var decay = TrExpRand(tr, 6, 15);
	var amp = TrRand(tr, 0.15, 0.5);
	var bottom = TrChoose(tr, [2, 5]);
	var top = TrRand(tr, 7, 9.1);
	var cFreq = e.x * 2300 + 120;
	var rq = samp + TrRand(tr, 0.1, 0.15);
	var rate = TrExpRand(tr, 0.1, 3);
