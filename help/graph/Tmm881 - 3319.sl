...
var pre = Voicer(16) { :e |
	var freq = (e.x * 25 + 48).MidiCps + [0, TrRand(e.w, 1, 4)];
	var carriers = [SinOsc(freq, 0), Saw(freq), Pulse(freq, 0.5)];
	var modulators = { SinOsc(TrRand(e.w, 0.0322, 1.0) * 0.0647, 0) } ! carriers.size;
	var filterEnv = Env([3600, 220, 3600], [e.y * 2, 16], 2, 1, nil, 0).asEnvGen(e.w);
	EqPan2(Lpf((carriers * modulators).sum, filterEnv), e.i * 2 - 1).sum * LagUd(e.w * e.z, 0, 0.25)
}.sum;
