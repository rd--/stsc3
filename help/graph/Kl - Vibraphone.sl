...
var voiceFunc = { :e |
	...
	var modFreq = e.j * 4 + 3;
	var modAmp = e.k * 2;
	var mod = LinLin(SinOsc(modFreq, TrRand(e.w, 0, 2 * pi)), -1, 1, 1 - ([0.5, 0.3, 0.3, 0.5] * modAmp), 1);
	...
	var zero = 0.000001;
	var soundMain = TrXLine(e.w, zero, 1, 1 / freqs) * TrXLine(e.w, 1, zero, decays * 4) * sound * mod;
	var soundHigh = TrXLine(e.w, 0.3 * (velocity + zero), zero, 0.02) * SinOsc(hiFreqs, 0); (* hi frequency onset *)
	var pan = e.i * 2 - 1;
